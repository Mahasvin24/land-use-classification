...
# 1. Define the official ESA WorldCover colors for your classes
# Order: [Forest, Shrubland, Grassland, Cropland, Built-up, Bare, Water]
class_colors = [
    '#006400', # 0: Forest
    '#ffbb22', # 1: Shrubland
    '#ffff4c', # 2: Grassland
...
# 2. Create a colorful version of the prediction map
from matplotlib.colors import ListedColormap
custom_cmap = ListedColormap(class_colors)

# 3. Save the image to the local Colab disk
output_filename = "SD_LandUse_Prediction.png"

# We use 'imsave' to save just the image data without axes/whitespace
plt.imsave(output_filename, prediction_map, cmap=custom_cmap)

#new model that can differentiate between agriculture and vegetation
...
# Use the exact names found via !gsutil ls
train_path_tex = 'gs://dslulc-training-data/land_use_train_v3.csv'
val_path_tex = 'gs://dslulc-training-data/land_use_val_v3.csv'

# 1. Load
df_train = pd.read_csv(train_path_tex)
df_val = pd.read_csv(val_path_tex)

# 2. Rename columns to ensure no hidden characters cause KeyErrors
# GEE export order: B2, B3, B4, B8, NDVI, entropy, clump_size, edge_density, lc
new_names = ['B2', 'B3', 'B4', 'B8', 'NDVI', 'entropy', 'clump_size', 'edge_density', 'lc']

if len(df_train.columns) == len(new_names):
    df_train.columns = new_names
    df_val.columns = new_names
else:
    print(f"Warning: File has {len(df_train.columns)} columns, but we expected {len(new_names)}.")
    print("Actual columns:", df_train.columns.tolist())

# 3. Features
features = ['B2', 'B3', 'B4', 'B8', 'NDVI', 'entropy', 'clump_size', 'edge_density']
target = 'lc'
...
# 6. Export the Model
model_filename = 'lulc_rf_texture_model.pkl'
...
print(f"Image saved to Colab as {output_filename}")

# 4. Trigger an automatic download to your computer
